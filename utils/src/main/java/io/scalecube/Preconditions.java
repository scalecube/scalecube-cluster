...
public final class Preconditions {
   ...
   * @param errorMessageTemplate a template for the exception message should the check fail. The message is formed by
   *        replacing each {@code %s} placeholder in the template with an argument. These are matched by position - the
   *        first {@code %s} gets {@code errorMessageArgs[0]}, etc. Unmatched arguments will be appended to the
  ...
  private static String format(String template, Object... args) {
    String template0 = String.valueOf(template); // null -> "null"
    // start substituting the arguments into the '%s' placeholders
    StringBuilder builder = new StringBuilder(template0.length() + 16 * args.length);
    int templateStart = 0;
    int idx = 0;
    while (idx < args.length) {
      int placeholderStart = template0.indexOf("%s", templateStart);
